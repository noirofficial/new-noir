...
QString formatBitcoinURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("noir:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("noir-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a noir.desktop file to the autostart directory:
