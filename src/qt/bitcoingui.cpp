...
void BitcoinGUI::createActions()
{
    ...
    if (!fLiteMode){
        masternodeAction = new QAction(QIcon(":/icons/connect_0"), tr("&Masternodes"), this);
    ...
    openAction->setStatusTip(tr("Open a noir: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Noir command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Noir network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
