...
{
    unsigned int nTargetLimit = UintToArith256(params.posLimit).GetCompact();

    // Genesis block or first proof-of-stake block
    if (pindexLast == NULL || pindexLast->nHeight == params.nLastPOWBlock)
...
unsigned int CalculateNextTargetRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params, bool fProofOfStake)
{
    ...
    const arith_uint256 bnTargetLimit = UintToArith256(params.posLimit);
    arith_uint256 bnNew;
    bnNew.SetCompact(pindexLast->nBits);
