...
//
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nBlockTime, CAmount prevoutValue, const COutPoint& prevout, unsigned int nTimeTx, bool fPrintProofOfStake)
{
      if (nTimeTx < nBlockTime)  // Transaction timestamp violation
        return false;
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBlockTime, unsigned int nBits, BlockValidationState &state, CCoinsViewCache& view)
{
    ...
    if(!view.GetCoin(txin.prevout, coinPrev))
        return state.Invalid(BlockValidationResult::BLOCK_INVALID_HEADER, "stake-prevout-diff-block-prevout", strprintf("CheckProofOfStake() : Stake prevout %s does not exist", txin.prevout.hash.ToString()));
...
bool CheckKernel(CBlockIndex* pindexPrev, unsigned int nBits, uint32_t nTime, const COutPoint& prevout, const std::map<COutPoint, CStakeCache>& cache, int64_t *pBlockTime, CCoinsViewCache& view)
{
    ...
    *pBlockTime = pindexPrev->GetBlockTime();
    ...
    if(it == cache.end()) {
        ...
        return CheckStakeKernelHash(pindexPrev, nBits, *pBlockTime, coinPrev.out.nValue, prevout, nTime);
    } else {
        //found in cache
        ...
        return CheckStakeKernelHash(pindexPrev, nBits, *pBlockTime, stake.amount, prevout, nTime);*/
    }
}
