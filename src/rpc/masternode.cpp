...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "outputs") {
        ...
        pwallet->AvailableCoins(*locked_chain, vPossibleCoins, true, nullptr, Params().GetConsensus().nMasternodeCollateralRequired * COIN, Params().GetConsensus().nMasternodeCollateralRequired * COIN, MAX_MONEY, 0, true);
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
            RPCHelpMan{"masternodelist",    
                ...
                "  payee          - Print NOIR address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
...
{ //  category              name                                actor (function)                argNames
    //  --------------------- ------------------------          -----------------------         ----------
    { "masternode",            "mnsync",                           &mnsync,                        {} },
    { "masternode",            "spork",                            &spork,                         {"name","value"} },
    { "masternode",            "masternodelist",                   &masternodelist,                {"mode","filter"} },
    { "masternode",            "sentinelping",                     &sentinelping,                  {"version"} }, 
    { "masternode",            "masternodebroadcast",              &masternodebroadcast,           {"command","data"} },
    { "masternode",            "masternode",                       &masternode,                    {"command","data"} },
};
// clang-format on
